...
// repo's own netstandard2.0 build of AnvilMap.Abstractions.dll.
...
";
    ...
    private static MetadataReference[] NetStandard20References()
    {
        ...
        var abstractionsBinDir = Path.Combine(FindRepoRoot(), "src", "AnvilMap.Abstractions", "bin");
        var abstractionsNetStandardDll = Directory.GetFiles(abstractionsBinDir, "AnvilMap.Abstractions.dll", SearchOption.AllDirectories)
            .FirstOrDefault(f => f.Contains(Path.Combine("netstandard2.0", "AnvilMap.Abstractions.dll")));

        Assert.True(abstractionsNetStandardDll is not null, $"Expected to find a netstandard2.0 AnvilMap.Abstractions.dll under {abstractionsBinDir} - build AnvilMap.Abstractions (netstandard2.0) first.");
    ...
    private static string FindRepoRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(dir.FullName, "AnvilMap.sln")))
            {
                return dir.FullName;
            }
        }

        throw new InvalidOperationException("Could not find AnvilMap.sln walking up from " + AppContext.BaseDirectory);
    }
