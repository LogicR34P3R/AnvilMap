...
// buildTransitive/AnvilMap.Generator.props auto-configures InterceptorsNamespaces for any
...
// AnvilMap.Generator.csproj and silently dropping the pack item for the props file.
public class PackagingTests
{
    ...
    public void Pack_IncludesBuildTransitivePropsFile_SettingInterceptorsNamespaces()
    {
        ...
        var csprojPath = Path.Combine(repoRoot, "src", "AnvilMap.Generator", "AnvilMap.Generator.csproj");
        var workDir = Path.Combine(Path.GetTempPath(), "AnvilMap.Generator.PackagingTest." + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            // broke restore for the ProjectReference to AnvilMap.Abstractions instead.
            ...
            var nupkgPath = Directory.GetFiles(outDir, "AnvilMap.Generator.*.nupkg").SingleOrDefault();
            Assert.False(string.IsNullOrEmpty(nupkgPath), $"No AnvilMap.Generator .nupkg produced in {outDir}.\nSTDOUT:\n{stdout}");

            using var archive = ZipFile.OpenRead(nupkgPath!);

            var propsEntry = archive.GetEntry("buildTransitive/AnvilMap.Generator.props");
            ...
            Assert.Contains("AnvilMap", content);
    ...
    private static string FindRepoRoot()
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "AnvilMap.sln")))
        {
            dir = dir.Parent;
        }
        ...
        if (dir is null)
        {
            throw new InvalidOperationException("Could not locate AnvilMap.sln by walking up from " + AppContext.BaseDirectory);
        }
