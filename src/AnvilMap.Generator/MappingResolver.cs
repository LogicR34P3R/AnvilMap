...
internal static partial class MappingResolver
{
        ...
        Action<Diagnostic>? report = null)
    {
        ...
        // ReportDuplicates (AM017) flags the ambiguity rather than resolving it silently.
        ...
        foreach (var destinationProperty in destination.GetMembers().OfType<IPropertySymbol>())
        {
            ...
            if (explicitConverters.TryGetValue(destinationProperty.Name, out var converterMethodName))
            {
                ...
                // here rather than silently emitted, so it surfaces as AM013 (unmapped required
            ...
            // rather than silently emitted, so it surfaces as AM013 (unmapped required property)
        ...
        // MappingEmitter can build `new Dest(args...)` instead of skipping the mapping (AM006).
    ...
    // unconditioned property of the exact same type - falling back to null (AM006 skip) on
    ...
    // type can hold null - every other combination reports AM019 instead of resolving silently.
    ...
    // populate the diagnostic Properties AnvilMap.CodeFixes reads.
